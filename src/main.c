...
typedef struct {
    ...
    Texture2D t;
    bool a;
} E; // enemy struct = pathetic sausage meat
...
typedef struct {
    ...
    Texture2D t;
    bool a;
} B; // bullets = flying death sticks, yes they're as shitty as they sound

int main(void)
{
    // no thats not the name of the game - InitWindow(SCREEN_W, SCREEN_H, "Block Shooter - Stupid Edition");
    InitWindow(SCREEN_W, SCREEN_H, "Block Shooter - Lightweight Port");
    SetTargetFPS(60);
    srand((unsigned int)time(NULL));
    InitAudioDevice();

    // FUCKING ,DON'T LOOK AT MY MESSY CODE
    ...
    E enemies[MAX_E] = { 0 };
    B bullets[MAX_B] = { 0 }; // arrays, because I can't do dynamic shit, sue me

    float spawnTimer = 0.0f;
    float spawnInterval = 1.5f; // spawn timer, or just another stupid float
    ...
    // what the fuck is this chunk of code
    UnloadTexture(P);
    UnloadTexture(G);
    UnloadTexture(Bu);
    UnloadTexture(En);
